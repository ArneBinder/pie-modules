...
class RETextClassificationWithIndicesTaskModule(
    ...
    parameters:
        ...
        argument_and_relation_type_whitelist: Dict[str, List[List[str]]], optional, defaults None. If set, only
            given relation types with given argument types will persist in documents and generated by
            `create_relation_candidates`. This also affects predictions on `decode()`, so it strictly filters both model
            input and output.
    """
    ...
    def create_annotations_from_output(
        ...
        if self.multi_label:
            ...
            # Create annotation only if neither
            if not (
                # it's a none_label annotation generated by `add_candidate_relations`
                (self.add_candidate_relations and label == self.none_label)
                or (  # Whitelist is defined and relation is not on a whitelist
                    self.argument_and_relation_type_whitelist is not None
                    and not (
                        label in self.argument_and_relation_type_whitelist
                        and (arg_labels in self.argument_and_relation_type_whitelist[label])
                    )
                )
            ):
