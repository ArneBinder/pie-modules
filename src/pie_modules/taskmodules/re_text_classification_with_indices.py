...
class RETextClassificationWithIndicesTaskModule(
    ...
    parameters:
        ...
        argument_and_relation_type_whitelist: Dict[str, List[List[str]]], optional, defaults None. If set, only
            ...
            `create_relation_candidates`. This also affects predictions on `decode()`, so it strictly filters
            both model input and output. Can also be passed as a list of lists, where the first element is
            the relation type and the rest are the argument types.
    """
