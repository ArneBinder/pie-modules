...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if prompt is not None:
            encoder_hidden_states = encoder_hidden_states + prompt.transpose(0, 1)
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        # self.embed_positions_replace2 = LearnedPositionalEmbedding(
    ...
    def forward(
        ...
        **unused,
        ...
        """Includes several features from "Jointly Learning to Align and Translate with Transformer
        ...
        if "pos_emb" in unused:
            pos_emb = unused.pop("pos_emb")
        else:
            pos_emb = None
        ...
        if pos_emb is not None:
            positions = self.embed_positions_replace(pos_emb, use_cache=use_cache, pos_id=True)
        else:
...
class Attention(nn.Module):
    ...
    def _shape(self, tensor, seq_len, bsz):
        return (
            tensor.contiguous().view(seq_len, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        )

    def _mf_shape(
        self, tensor, encode_len, bsz, decode_len=None
    ):  # -->>> <head*bsz, decode_len, -1, encode_len>
        tensor = tensor.transpose(1, 2)
        decode_len = tensor.size(0)
        return (
            tensor.contiguous()
            .view(decode_len, encode_len, bsz * self.num_heads, self.head_dim)
            .transpose(1, 2)
        )
    ...
    def forward(
        ...
        if layer_state is not None:  # reuse k,v and encoder_padding_mask
            ...
            if saved_state is None:
                raise ValueError("saved_state is None")
        ...
        q = self.q_proj(query) * self.scaling  # <encode_len,bsz,dim>
        if static_kv:
            if key is None:
                k = v = None
            else:  # decoding
                k = self.k_proj(key)  # <encode_len,bsz*beam,dim>
                v = self.v_proj(key)

        else:  # encoding self attention
            k = self.k_proj(query)  # <input_len, bsz, emb_dim>
            v = self.v_proj(query)

        q = self._shape(
            q, tgt_len, bsz
        )  # <bsz,decode_dim,dim> ---->>>> <bsz * self.num_heads, decode_dim, self.head_dim>
        ...
        src_len = k.size(1)
        attn_weights = torch.bmm(q, k.transpose(1, 2))  # add mention flag need for
        ...
        attn_output = torch.bmm(attn_probs, v)  # add mention flag need for
...
class LearnedPositionalEmbedding(nn.Embedding):
    ...
    Padding ids are ignored by either offsetting based on padding_idx or by setting padding_idx to
    None and ensuring that the appropriate position ids are passed to the forward function.
    """
    ...
    def forward(self, input_ids, use_cache=False, pos_id=False):
        ...
        if pos_id:
            if use_cache:
                positions = input_ids.data.new(1, 1).fill_(input_ids[0, -1])
            else:
                positions = input_ids
        else:
            if use_cache:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not isinstance(encoder_outputs, BaseModelOutput):
            raise TypeError("encoder_outputs needs to be of type BaseModelOutput")
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def forward(
        ...
        **unused,
        ...
        `optional`): Labels for computing the masked language modeling loss. Indices should either
        be in ``[0, ..., config.vocab_size]`` or -100 (see ``input_ids`` docstring). Tokens with
        indices set to ``-100`` are ignored (masked), the loss is only computed for the tokens with
