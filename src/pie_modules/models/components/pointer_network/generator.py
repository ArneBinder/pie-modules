...
from .interface import Seq2SeqDecoder
from .utils import _get_model_device, seq_len_to_mask
...
class SequenceGenerator:
    ...
    def set_new_generator(
        ...
        self.generate_func = partial(
            ...
            restricter=restricter,
            decode_mask=self.decode_mask,
        )

    @torch.no_grad()
    def generate(self, state, src_seq_len=None, tokens=None, ner_tag=None):
        """
        ...
        return self.generate_func(
            tokens=tokens, state=state, ner_tag=ner_tag, src_seq_len=src_seq_len
        )
...
def greedy_generate(
    ...
    :param int max_length: 生成句子的最大长度, 每句话的decode长度为max_length + max_len_a*src_len
    :param float max_len_a: 每句话的decode长度为max_length + max_len_a*src_len。
        如果不为0，需要保证State中包含encoder_mask
...
def _beam_search_generate(
    ...
    if state.num_samples:
        ...
        ), "The number of samples in `tokens` and `state` should match."
    ...
    assert (
        vocab_size >= num_beams
    ), "num_beams should be smaller than the number of vocabulary size."
    ...
    # ner_tag = ner_tag.repeat_interleave(4,0)
    ...
    if max_len_a != 0:
        ...
        if state.encoder_mask is not None:
            ...
            max_lengths = tokens.new_full(
                (batch_size * num_beams,), fill_value=max_length, dtype=torch.long
            )
        real_max_length = max_lengths.max().item()
    else:
        ...
        if state.encoder_mask is not None:
            max_lengths = (
                state.encoder_mask.new_ones(state.encoder_mask.size(0)).long() * max_length
            )
        else:
            max_lengths = tokens.new_full(
                (batch_size * num_beams,), fill_value=max_length, dtype=torch.long
    ...
    hypos = [
        BeamHypotheses(num_beams, real_max_length, length_penalty, early_stopping=False)
    ...
    batch_inds_with_numbeams_interval = (
        (torch.arange(batch_size) * num_beams).view(-1, 1).to(token_ids)
    )
    ...
    while cur_len < real_max_length:
        ...
        if decode_mask:
            if decode_idx == 3:
                ...
                mask_e = torch.cat([eoc, torch.tensor([vocab_size]).to(device)])
                mask_e = seq_len_to_mask(mask_e)[:-1]
                mask_s = torch.cat([soc, torch.tensor([vocab_size]).to(device)])
                mask_s = seq_len_to_mask(mask_s)[:-1]
                mask = mask_s ^ mask_e
                mask[:, none_ids] = False
                scores = scores.masked_fill(mask, -1e32)

            if decode_idx == 1:
                soc = token_ids[:, -1].squeeze()
                mask = torch.cat([soc, torch.tensor([vocab_size]).to(device)])
                mask = seq_len_to_mask(mask)[:-1]
                mask[:, none_ids] = False
                scores = scores.masked_fill(mask, -1e32)
            ...
            if decode_idx == 4:
                ...
                start_of_current_span = token_ids[:, -1]
                end_of_ahead_span = token_ids[:, -3]
                start_of_ahead_span = token_ids[:, -4]

                start_position = end_of_ahead_span < start_of_current_span
                for idx, skip in enumerate(start_position):
                    if not skip:
                        scores[idx, start_of_ahead_span[idx] :] = -1e32
                # eoc =token_ids[:,-3].squeeze()
                # soc =token_ids[:,-4].squeeze()
                # mask_e = torch.cat([eoc,torch.tensor([vocab_size]).to(device)])
                # mask_e = seq_len_to_mask(mask_e)[:-1]
                # mask_s = torch.cat([soc,torch.tensor([vocab_size]).to(device)])
                # mask_s = seq_len_to_mask(mask_s)[:-1]
                ...
                mask = scores.new_full(scores.size(), fill_value=0).eq(0)
                mask[:, span_ids] = False
                mask[:, none_ids] = False
                scores = scores.masked_fill(mask, -1e32)
            elif decode_idx == 2:
                ...
                mask = scores.new_full(scores.size(), fill_value=0).eq(0)
                mask[:, relation_ids] = False
                mask[:, none_ids] = False
                scores = scores.masked_fill(mask, -1e32)
            else:
                mask = scores.new_full(scores.size(), fill_value=0).eq(1)
                mask[:, relation_ids] = True
                mask[:, span_ids] = True
                mask[:, none_ids] = False
                scores = scores.masked_fill(mask, -1e32)
        ...
        if repetition_penalty != 1.0:
            ...
            token_scores = (
                lt_zero_mask * repetition_penalty * token_scores
        ...
        if cur_len + 1 == real_max_length:
            eos_batch_idx = (
                ...
                .to(next_tokens)
                .repeat_interleave(repeats=num_beams, dim=0)
            )
        ...
        if flag:
            ...
            for batch_idx, beam_ind, beam_idx in zip(
                ...
                if not dones[batch_idx]:
                    ...
                    if _eos_token_id != -1:
                        hypos[batch_idx].add(
                            _token_ids[batch_idx * num_beams + beam_idx, :cur_len].clone(), score
                        )
                    else:
                        hypos[batch_idx].add(
                            _token_ids[batch_idx * num_beams + beam_idx].clone(), score
                        )

        # 更改state状态, 重组token_ids
        reorder_inds = (batch_inds_with_numbeams_interval + _from_which_beam).view(
            -1
        )  # flatten成一维
        state.reorder_state(reorder_inds)
        # 重新组织token_ids的状态
        token_ids = torch.cat(
            [token_ids.index_select(index=reorder_inds, dim=0), _next_tokens], dim=-1
        )
    ...
    for i, hypotheses in enumerate(hypos):
        if len(hypotheses.hyp) > 0:
            ...
            best_hyp = torch.tensor([0, 1]).to(device)
        # 把上面替换为非eos的词替换回eos
        if _eos_token_id != -1:
